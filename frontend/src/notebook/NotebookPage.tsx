...
export function NotebookPageContainer({
  confirmAction,
  registerSemHandlers,
  shellConfig,
  ws,
}: NotebookPageContainerProps) {
  const {
    document,
    error,
    handleAskAI,
  ...
  return (
    <NotebookPageView
      ...
      onClearNotebook={() => {
        void handleClearNotebook();
      }}
      onCreateCodeCell={() => {
        void handleInsertBelow(lastCellId, "code");
      }}
      onCreateMarkdownCell={() => {
        void handleInsertBelow(lastCellId, "markdown");
      }}
      onPersistTitle={(title) => {
        void persistTitle(title);
      }}
      onResetKernel={() => {
        void handleResetKernel();
      }}
      renderedCells={
        document ? (
